...
public class TrademarkRenewalReceipt(Filling model, string appId) : IDocument
{
    string nairaSymbol = "\u20A6";
        ...
        void ComposeContent(IContainer container)
        {
            container
                ...
                .Column(column =>
                {
                    ...
                    column.Item().AlignCenter().Text("RENEWAL PAYMENT RECEIPT").FontColor(Colors.Green.Darken3).FontFamily(Fonts.TimesNewRoman).FontSize(16).ExtraBold();
                    column.Item().Height(25);
                     
                    var application = model.ApplicationHistory.FirstOrDefault(a=>a.id == appId);
                    ...
                    column.Item().Table(table =>
                    {
                        ...
                        table.Cell().Element(Block).Column(c => {
                            ...
                            c.Item().Text(app?.FilingDate ?? application?.ApplicationDate.ToString("dd MMMM, yyyy")).FontSize(12).FontFamily(Fonts.TimesNewRoman);
                        });
                        table.Cell().Element(Block).Column(c => {
                            c.Item().Text("Payment rrr:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                            c.Item().Text(app?.rrr ?? application?.PaymentId).FontSize(12).FontFamily(Fonts.TimesNewRoman);
                        });
                        table.Cell().Element(Block).Column(c => {
                        ...
                        table.Cell().Element(Block).Column(c => {
                            ...
                            c.Item().Text($"{nairaSymbol}15,500").FontFamily(Fonts.TimesNewRoman);
                        });
                        table.Cell().Element(Block).Column(c => {
                        ...
                        table.Cell().Element(Block).Column(c => {
                            ...
                            c.Item().Text("CLD0012").FontSize(12).FontFamily(Fonts.TimesNewRoman);
                        });
                    });
