...
{
    public class AssignmentAcknowledgement(Filling model, byte[] image, string applicationId) : IDocument
    {
        //string nairaSymbol = "\u20A6";
        ...
        void ComposeContent(IContainer container)
        {
            ...
            container
                ...
                .Column(column =>
                {
                    ...
                    column.Item().AlignCenter().Text("ASSIGNMENT ACKNOWLEDGEMENT LETTER")
                    ...
                    var regUser = model.Assignees?.FirstOrDefault(r => r.Id == applicationId);
                    ...
                    column.Item().Table(table =>
                    {
                        string filingDate = string.IsNullOrWhiteSpace(app.FilingDate)
                            ? "N/A"
                            : DateTime.Parse(app.FilingDate).ToString("dd MMMM, yyyy");
                        ...
                        table.Cell().Element(Block).Column(c =>
                        {
                            ...
                            c.Item().Text("Registration of Assignment within the Prescribed Period").FontSize(12)
                    ...
                    column.Item().Table(table =>
                    {
                        ...
                        table.Cell().ColumnSpan(2).Element(HeaderElement).Text("ASSIGNOR INFORMATION")
                            .FontFamily(Fonts.TimesNewRoman).FontSize(14).Bold();
                        table.Cell().Element(Block).Column(c =>
                        {
                    ...
                    column.Item().Table(table =>
                    {
                        ...
                        table.Cell().ColumnSpan(2).Element(HeaderElement).Text("ASSIGNEE INFORMATION")
                            .FontFamily(Fonts.TimesNewRoman).FontSize(14).Bold();
                        table.Cell().Element(Block).Column(c =>
                        {
                            c.Item().Text("Name:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                            c.Item().Text(regUser?.Name).FontSize(12).FontFamily(Fonts.TimesNewRoman);
                        });
                        table.Cell().Element(Block).Column(c =>
                        {
                            c.Item().Text("Email:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                            c.Item().Text(regUser?.Email).FontSize(12).FontFamily(Fonts.TimesNewRoman);
                        });
                        table.Cell().Element(Block).Column(c =>
                        {
                            c.Item().Text("Phone Number:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                            c.Item().Text(regUser?.Phone).FontSize(12).FontFamily(Fonts.TimesNewRoman);
                        });
                        table.Cell().Element(Block).Column(c =>
                        {
                            c.Item().Text("Nationality:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                            c.Item().Text(regUser?.Nationality).FontSize(12).FontFamily(Fonts.TimesNewRoman);
                        });
                        table.Cell().ColumnSpan(2).Element(Block).Column(c =>
                        {
                            c.Item().Text("Address:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                            c.Item().Text(regUser?.Address).FontSize(12).FontFamily(Fonts.TimesNewRoman);
                        });
                    });
