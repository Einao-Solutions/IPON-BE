...
{
    public class CertificateAcknowledgement(Filling model, byte[] image, Receipt receipt) : IDocument
    {
        string nairaSymbol = "\u20A6";
        ...
        void ComposeContent(IContainer container)
        {
            ...
            container
                ...
                .Column(column =>
                {
                    ...
                    column.Item().AlignCenter().Text("TRADEMARK CERTIFICATE ACKNOWLEDGEMENT LETTER").FontColor(Colors.Green.Darken3).FontFamily(Fonts.TimesNewRoman).FontSize(16).ExtraBold();
                    ...
                    column.Item().Table(table =>
                    {
                        ...
                        table.Cell().Element(Block).Column(c => {
                            c.Item().Text("Certificate Payment Date:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                        ...
                        table.Cell().Element(Block).Column(c => {
                            ...
                            c.Item().Text(model.ApplicationHistory[0].ApplicationDate.ToString("dd MMMM, yyyy")).FontSize(12).FontFamily(Fonts.TimesNewRoman);
                        });table.Cell().Element(Block).Column(c => {
                            c.Item().Text("New Application Payment ID:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                            c.Item().Text(model.ApplicationHistory[0].PaymentId ?? "").FontSize(12).FontFamily(Fonts.TimesNewRoman);
                        });
                        table.Cell().Element(Block).Column(c => {
                            c.Item().Text("Certificate Payment ID:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                        ...
                        table.Cell().ColumnSpan(2).Element(Block).Column(c => {
                            ...
                            c.Item().Text("CLD004").FontSize(12).FontFamily(Fonts.TimesNewRoman);
                        });
                    });
                    ...
                    column.Item().Table(table =>
                    {
                        ...
                        table.Cell().Element(Block).Column(c => {
                                ...
                                image != null && image.Length > 0)
                            {
                                ...
                                {
                                    c.Item().Text("Invalid image data").FontSize(12).FontColor(Colors.Red.Medium).FontFamily(Fonts.TimesNewRoman);
                                }
                            }
                            else
                            {
