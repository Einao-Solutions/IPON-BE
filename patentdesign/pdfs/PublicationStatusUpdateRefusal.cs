...
{
    public class PublicationStatusUpdateRefusal(Filling model, byte[] image, ApplicationInfo selectedHistory) : IDocument
    {
        ...
        void ComposeContent(IContainer container)
        {
            ...
            container
                ...
                .Column(column =>
                {
                    ...
                    {
                        FileTypes.TradeMark => "PUBLICATION STATUS UPDATE REFUSAL LETTER",
                    ...
                    column.Item().Table(table =>
                    {
                        ...
                        table.Cell().ColumnSpan(2).Element(HeaderElement).Text("REJECTION INFORMATION").FontFamily(Fonts.TimesNewRoman).FontSize(14).Bold();

                        table.Cell().Element(Block).Column(c =>
                        {
                            c.Item().Text("officers Name:").FontSize(10).FontFamily(Fonts.TimesNewRoman).Bold();
                            c.Item().Text(model.ApplicationHistory.LastOrDefault()?.StatusHistory.LastOrDefault(s => s.afterStatus == ApplicationStatuses.Rejected)?.User ?? "Populate here").FontSize(12).FontColor(Colors.Black).FontFamily(Fonts.TimesNewRoman).Italic();
                        });
                        table.Cell().Element(Block).Column(c =>
                        {
                    ...
                    column.Item().Height(10);
                    column.Item().AlignCenter().Text("YOUR APPLICATION IS REJECTED").FontColor(Colors.Green.Darken2).FontFamily(Fonts.TimesNewRoman).FontSize(14).Bold();
