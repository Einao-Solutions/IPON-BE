...
public class ChangeOfAddressCert(Filling model, string url, byte[]? imageData, string applicationId, Signatory signature): IDocument
{
        ...
        private void ComposeContent(IContainer container)
        {
            ...
            container.PaddingVertical(5).Column(column =>
            {
                ...
                column.Item().Table(table =>
                {
                    ...
                    // ===== NAME =====
                    table.Cell().Element(Cell)
                        .Text(!string.IsNullOrWhiteSpace(app.OldAddress) ? app.OldAddress : (applicants?.Address ?? "N/A"))
                        .FontFamily(Fonts.TimesNewRoman);
