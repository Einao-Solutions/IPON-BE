...
{
    public class StatusSearchReport(Filling model, byte[] image) : IDocument
    {
        ...
        void ComposeContent(IContainer container)
        {
            ...
            container
                ...
                .Column(column =>
                {
                    ...
                    {
                        FileTypes.TradeMark => "TRADEMARK STATUS REPORT",
                    ...
                    column.Item().Table(table =>
                    {
                        ...
                        table.Cell().Element(Block).Column(c =>
                        {
                            c.Item().Text("RTM No:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                            c.Item().Text(model.RtmNumber ?? "").FontSize(12).FontColor(Colors.Black).FontFamily(Fonts.TimesNewRoman);
                        });
                        table.Cell().Element(Block).Column(c =>
                        {
                            c.Item().Text("Filing Date:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                            c.Item().Text(model.DateCreated.ToString("dd MMMM, yyyy")).FontSize(12).FontColor(Colors.Black).FontFamily(Fonts.TimesNewRoman);
                        });
                        table.Cell().Element(Block).Column(c =>
                        {
                    ...
                    column.Item().Table(table =>
                    {
                        ...
                        table.Cell().Element(Block).Column(c =>
                        {
                            ...
                            c.Item().Text(applicant.Name ?? "").FontSize(12).FontColor(Colors.Black).FontFamily(Fonts.TimesNewRoman);
                        });
                        table.Cell().Element(Block).Column(c =>
                        {
                            c.Item().Text("Email:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                            c.Item().Text(applicant.Email ?? "").FontSize(12).FontColor(Colors.Black).FontFamily(Fonts.TimesNewRoman);
                        });
                        table.Cell().Element(Block).Column(c =>
                        {
                            c.Item().Text("Phone Number:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                            c.Item().Text(applicant.Phone ?? "").FontSize(12).FontColor(Colors.Black).FontFamily(Fonts.TimesNewRoman);
                        });
                        table.Cell().Element(Block).Column(c =>
                        {
                            c.Item().Text("Nationality:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                            c.Item().Text(applicant.country ?? "").FontSize(12).FontColor(Colors.Black).FontFamily(Fonts.TimesNewRoman);
                        });
                        table.Cell().ColumnSpan(2).Element(Block).Column(c =>
                        {
                            c.Item().Text("Address:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                            c.Item().Text(applicant.Address ?? "").FontSize(12).FontColor(Colors.Black).FontFamily(Fonts.TimesNewRoman);
                        });
                    });
                        ...
                        case FileTypes.TradeMark:
                            column.Item().Table(table =>
                            {
                                ...
                                table.Cell().Element(Block).Column(c =>
                                {
                                    ...
                                    c.Item().Text(model.TitleOfTradeMark ?? "").FontSize(12).FontColor(Colors.Black).FontFamily(Fonts.TimesNewRoman);
                                });
                                table.Cell().Element(Block).Column(c =>
                                {
                                    c.Item().Text("Class:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                                    c.Item().Text(model.TrademarkClass?.ToString() ?? "").FontSize(12).FontColor(Colors.Black).FontFamily(Fonts.TimesNewRoman);
                                });
                                table.Cell().Element(Block).Column(c =>
                                {
                                    c.Item().Text("Description of Goods:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                                    c.Item().Text(model.TrademarkClassDescription ?? "").FontSize(12).FontColor(Colors.Black).FontFamily(Fonts.TimesNewRoman);
                                });
                                table.Cell().Element(Block).Column(c =>
                                {
                                ...
                                table.Cell().ColumnSpan(2).Element(Block).Column(c =>
                                {
                                    ...
                                    c.Item().Text(model.TrademarkDisclaimer ?? "").FontSize(12).FontColor(Colors.Black).FontFamily(Fonts.TimesNewRoman);
                                });
                            });
                            break;
