...
{
    public class AssignmentCertificate(AssignmentCertificateType assDets, FileTypes type) : IDocument
    {
       ...
       void ComposeContent(IContainer container)
        {
             container
                ...
                .Column(column =>
                {
                    ...
                    column.Item().AlignCenter().Text("CERTIFICATE OF ASSIGNMENT").FontFamily(Fonts.TimesNewRoman).FontSize(16).Bold();
                    ...
                    column.Item().Height(5);
                    column.Item().Text($"I hereby notify you that a change has been recorded in favour of {type.ToString()}: {assDets.fileNumber+ConstantValues.AssPassage2}");
                    column.Item().Height(5);
                    ...
                    column.Item().Height(5);
                    column.Item().Text($"This assignment was made Pursuant to the deed of assignment dated: {assDets.assignmentType.dateOfAssignment.ToString("D")} received on: {assDets.paymentDate.ToString("D")} for application dated: {assDets.paymentDate.ToString("D")}").Style(TextStyle.Default.Bold());
