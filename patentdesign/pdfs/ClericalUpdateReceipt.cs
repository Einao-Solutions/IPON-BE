...
public class ClericalUpdateReceipt(Filling model, string appId) : IDocument
{
    string nairaSymbol = "\u20A6";
    ...
    void ComposeContent(IContainer container)
    {
        container
            ...
            .Column(column =>
            {
                ...
                column.Item().AlignCenter().Text("CLERICAL UPDATE PAYMENT RECEIPT").FontColor(Colors.Green.Darken3)
                ...
                column.Item().Table(table =>
                {
                    ...
                    string filingDate = app.ApplicationDate.ToString("dd MMMM, yyyy");
                    ...
                    table.Cell().Element(Block).Column(c =>
                    {
                        ...
                        c.Item().Text($"{nairaSymbol}2,000").FontFamily(Fonts.TimesNewRoman);
                    });
                    table.Cell().Element(Block).Column(c =>
                    {
                        c.Item().Text("Fee Title:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                        c.Item().Text("Application for Correction of Clerical Errors for Trademark").FontSize(12)
                            .FontFamily(Fonts.TimesNewRoman);
                    });
                    table.Cell().Element(Block).Column(c =>
                    {
                        c.Item().Text("Fee Code:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                        c.Item().Text("CLD0062").FontSize(12).FontFamily(Fonts.TimesNewRoman);
                    });
                });
                ...
                column.Item().Table(table =>
                {
                    ...
                    table.Cell().ColumnSpan(2).Element(HeaderElement).Text("CORRESPONDENCE INFORMATION")
                        .FontFamily(Fonts.TimesNewRoman).FontSize(14).Bold();
                    table.Cell().Element(Block).Column(c =>
                    {
