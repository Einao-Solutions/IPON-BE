...
{
    public class TrademarkCtcAcknowledgement : IDocument
    {
        ...
        public TrademarkCtcAcknowledgement(Filling model, string url, Receipt receipt, string appId)
        {
            this.model = model;
            this.url = url;
        ...
        private void ComposeContent(IContainer container)
        {
            container.Column(col =>
            {
                ...
                col.Item().AlignCenter().PaddingBottom(10)
                    ...
                    .FontFamily(Fonts.TimesNewRoman).FontSize(14);
                col.Item().AlignCenter()
                    .Text("TRADEMARK CTC ACKNOWLEDGEMENT LETTER")
                ...
                if (ctcRecordal != null && ctcRecordal.RequestedAttachments != null && ctcRecordal.RequestedAttachments.Count > 0)
                {
                    col.Item().Element(Header).Text("REQUESTED CERTIFIED TRUE COPIES")
                        .FontFamily(Fonts.TimesNewRoman).FontSize(14).Bold();
                    
                    foreach (var attachmentName in ctcRecordal.RequestedAttachments)
                    {
                        FullWidthBox(col, string.Empty, attachmentName);
                    }
                }
