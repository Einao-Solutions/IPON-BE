...
{
    public class DesignAmendmentReceipt(Receipt receipt, string url, Filling model) : IDocument
    {
        string nairaSymbol = "\u20A6";
        ...
        private void ComposeContent(IContainer container)
        {
            container.Column(col =>
            {
                ...
                if (!string.IsNullOrWhiteSpace(receipt.Amount) && decimal.TryParse(receipt.Amount, out var parsedAmount))
                {
                ...
                col.Item().AlignCenter().Text("DESIGN AMENDMENT RECEIPT").FontFamily(Fonts.TimesNewRoman).FontSize(16).FontColor(Colors.Green.Darken2).ExtraBold();
                col.Item().Height(10);
                ...
                // Footer
                col.Item().AlignCenter().PaddingTop(20).Text("PLEASE KEEP THIS RECEIPT FOR FUTURE REFERENCE")
