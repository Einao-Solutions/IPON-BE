...
{
    /// <summary>
    /// Generates the two?page Patent Acceptance Letter PDF that visually matches the
    /// "StatusSearchReport" sample layout (stacked label?value pairs inside bordered
    /// boxes with section headers spanning two columns).
    /// </summary>
    public class AcceptanceModelPatent : IDocument
    {
        /*???????????????????????????????????????????????
         *  STATE
         *??????????????????????????????????????????????*/
        ...
        private void ComposeContent(IContainer container)
        {
            container.Column(col =>
            {
                ...
                col.Item().AlignCenter().Text("PATENT ACCEPTANCE LETTER").FontFamily(Fonts.TimesNewRoman).FontSize(16).FontColor(Colors.Green.Darken3).ExtraBold();
                col.Item().Height(10);
                ...
                TwoColumnSection(col, "FILE INFORMATION", new[]
                {
                    ...
                    ("Acceptance Date:",  F(approvalDate)),
                ...
                ///*???????? DRAWING ?????????*/
                // DRAWING (only render if there is at least one "drawings" attachment)
                ...
                //EXAMINER INFO
                col.Item().Element(Header).Text("EXAMINER INFORMATION").FontFamily(Fonts.TimesNewRoman).FontSize(14).Bold();
                FullWidthBox(col, "Name:", F(model.ApplicationHistory?.FirstOrDefault()?.StatusHistory?.FirstOrDefault(sh => sh.beforeStatus == ApplicationStatuses.AwaitingExaminer)?.User));
