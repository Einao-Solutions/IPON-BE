...
public class ClericalUpdateAck(Filling model, byte[] image, string applicationId): IDocument
{
     //string nairaSymbol = "\u20A6";
        ...
        void ComposeContent(IContainer container)
        {
            ...
            container
                ...
                .Column(column =>
                {
                    ...
                    column.Item().AlignCenter().Text("CLERICAL UPDATE ACKNOWLEDGEMENT LETTER").FontColor(Colors.Green.Darken3).FontFamily(Fonts.TimesNewRoman).FontSize(16).ExtraBold();
                    column.Item().Height(25);

                    var history = model.ApplicationHistory.FirstOrDefault(a => a.id == applicationId);
                    var clerical = model.ClericalUpdates?.FirstOrDefault(c => c.Id == history.id);

                    //Payment Information Section
                    column.Item().Table(table =>
                    {
                        string filingDate = (history.ApplicationDate).ToString("dd MMMM, yyyy");
                        ...
                        table.Cell().Element(Block).Column(c =>
                        {
                            ...
                            c.Item().Text("Application for Correction of Clerical Errors for Trademark").FontSize(12).FontFamily(Fonts.TimesNewRoman);
                        });
                    ...
                    column.Item().Table(table =>
                    {
                        ...
                        table.Cell().ColumnSpan(2).Element(HeaderElement).Text("CLERICAL UPDATE INFORMATION").FontFamily(Fonts.TimesNewRoman).FontSize(14).Bold();
                        table.Cell().Element(Block).Column(c =>
                        {
                           // c.Item().Text().FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                            c.Item().Text("Clerical Update Type:").FontSize(12).FontFamily(Fonts.TimesNewRoman);
                        });
                        table.Cell().Element(Block).Column(c =>
                        {
                            //c.Item().Text("Email:").FontSize(10).FontFamily(Fonts.TimesNewRoman).SemiBold();
                            c.Item().Text(FormatPropertyName(clerical.UpdateType)).FontSize(12).FontFamily(Fonts.TimesNewRoman);
                        });
                        if (clerical != null)
                        {
