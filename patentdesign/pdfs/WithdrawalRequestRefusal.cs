...
{
    public class WithdrawalRequestRefusal(Filling model, byte[] image, ApplicationInfo selectedHistory) : IDocument
    {
        ...
        void ComposeContent(IContainer container)
        {
            ...
            container
                ...
                .Column(column =>
                {
                    ...
                    {
                        FileTypes.TradeMark => "TRADEMARK WITHDRAWAL REFUSAL LETTER",
                        FileTypes.Patent => "PATENT WITHDRAWAL REFUSAL LETTER",
                        FileTypes.Design => "DESIGN WITHDRAWAL REFUSAL LETTER",
                        _ => "WITHDRAWAL REFUSAL LETTER"
                    };
                    ...
                    column.Item().Table(table =>
                    {
                        ...
                        table.Cell().ColumnSpan(2).Element(HeaderElement).Text("REFUSAL INFORMATION").FontFamily(Fonts.TimesNewRoman).FontSize(14).Bold();
                        table.Cell().Element(Block).Column(c =>
                        {
                            c.Item().Text("Officer's Name:").FontSize(10).FontFamily(Fonts.TimesNewRoman).Bold();
                            var withdrawalApp = model.ApplicationHistory?
                             .FirstOrDefault(a => a.ApplicationType == FormApplicationTypes.WithdrawalRequest);

                            var officerName = withdrawalApp?
                                .StatusHistory?
                                .LastOrDefault(s => s.afterStatus == ApplicationStatuses.Rejected)?.User;
                            c.Item().Text(officerName ?? "N/A").FontSize(12).FontColor(officerName == null ? Colors.Black : Colors.Black).FontFamily(Fonts.TimesNewRoman).Italic();
                        });
                        table.Cell().Element(Block).Column(c =>
                        {
                            c.Item().Text("Comment:").FontSize(10).FontFamily(Fonts.TimesNewRoman).Bold();
                            c.Item().Text(model.WithdrawalReason ?? "N/A").FontSize(12).FontColor(model.WithdrawalReason == null ? Colors.Black : Colors.Black).FontFamily(Fonts.TimesNewRoman).Italic();
                        });
                    });

                    column.Item().Height(10);
                    column.Item().AlignCenter().Text("YOUR APPLICATION IS REFUSED").FontColor(Colors.Green.Darken2).FontFamily(Fonts.TimesNewRoman).FontSize(14).Bold();
                });
        }
    }
}
