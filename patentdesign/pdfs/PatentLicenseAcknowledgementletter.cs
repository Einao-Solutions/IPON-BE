...
{
    public class PatentLicenseAcknowledgementletter : IDocument
    {
        private readonly Filling model;
        private readonly string url;
        private readonly Receipt receipt;

        public PatentLicenseAcknowledgementletter(Filling model, string url, Receipt receipt)
        {
            this.model = model;
            this.url = url;
        ...
        private void ComposeContent(IContainer container)
        {
            container.Column(col =>
            {
                ...
                col.Item().AlignCenter().PaddingBottom(10)
                    ...
                    .FontFamily(Fonts.TimesNewRoman).FontSize(14);
                col.Item().AlignCenter()
                    .Text("PATENT LICENSE ACKNOWLEDGEMENT LETTER")
                ...
                // Get corresponding post-registration license app
                var licenseRecordal = model.PostRegApplications?
                    .FirstOrDefault(p => p.RecordalType == "Patent License Recordal");

                // LICENSOR INFORMATION (old licensor fields from post-reg app)
                if (licenseRecordal != null)
                {
                    ...
                    // LICENSEE INFORMATION (new licensee fields on same recordal)
